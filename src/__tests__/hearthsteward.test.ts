...
function makeState(buildings: Record<string, Building>, withHearthsteward: boolean): GameState {
  return {
    buildings,
    specialists: createInitialSpecialists(),
    globalSpecialistStorage: withHearthsteward ? ['spec_14'] : [],
  } as unknown as GameState;
}

describe('Hearthsteward housing cap bonus', () => {
  it('adds bonus cap to Farm and Patrician House, but not Stronghold', () => {
    ...
    const state = makeState({ [farm.id]: farm, [house.id]: house, [stronghold.id]: stronghold }, true);
  ...
  it('allows Farm growth to base cap + 1 when Hearthsteward is active', () => {
    ...
    const state = makeState({ [farm.id]: farm }, true);
