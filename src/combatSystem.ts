...
export function resolveAttack(
  ...
  if (defenderDead) {
    ...
    // DISTRACTION: permanently reduce defender's DEF and the attacker ranger's own ATK on each hit
    if (attacker.tags.includes(UnitTag.DISTRACTION)) {
      ...
      attacker.stats.attack = Math.max(0, attacker.stats.attack - Math.abs(ABILITIES.DISTRACTION_ATTACK_MOD));
    }
